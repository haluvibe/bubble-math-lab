...
export default function BubbleMathLab() {
  ...
  const targetsRowRef = useRef(null);
  const targetElementsRef = useRef({}); // Map target id -> DOM element
  ...
  // Get a target's real position relative to bubble-container
  ...
  // Backward-compat helper: get Y for the first uncaptured target (used by connection line, etc.)
  ...
  const performCapture = useCallback((bubble, target) => {
    // Show capture effect at the real target position
    const pos = getTargetPosition(target);
    setCaptureEffect({
      id: Date.now(),
      x: pos.x,
  ...
  const handleDragMove = useCallback((e) => {
    ...
    // Allow dragging above the container (negative Y) to reach the targets row
